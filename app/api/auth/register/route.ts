...
export async function POST(request: Request) {
  ...
  const upstream = await fetch(`${base}/api/v1/auth/register`, { method: "POST", headers: { Accept: "application/json", "Content-Type": "application/json" }, body: await request.text(), cache: "no-store" });
