...
export default function Home() {
  ...
  async function logout() {
    ...
    window.localStorage.removeItem("fc-manager-session");
    window.sessionStorage.removeItem("fc-manager-session");
  ...
  async function equipItem(item: PlayerItem) {
    ...
    if (!response.ok) { notify(payload.message || "Não foi possível equipar o item pela API."); return; }
  ...
  async function buyPixPackage(packageName: "starter" | "popular" | "value") {
    ...
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) { notify(payload.message || "Não foi possível criar o pedido PIX."); return; }
    setPendingPixOrder(payload.order);
    notify(`Pedido PIX #${payload.order.id} criado. Aguarde a confirmação do pagamento.`);
  }
  ...
  async function buyItem(item: PlayerItem) {
    ...
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) { notify(payload.message || "Não foi possível comprar o item."); return; }
    notify(`${item.name} adicionado ao inventário pela API.`);
  }

  async function buySelectedPlayer() {
    if (!selectedPlayer) return;
    const response = await fetch(`/api/game/v1/transfers/${selectedPlayer.id}/buy`, { method: "POST", headers: { Accept: "application/json", "Content-Type": "application/json", "Idempotency-Key": `player-${selectedPlayer.id}-${crypto.randomUUID()}` }, body: JSON.stringify({ payment_method: "coins" }) });
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) { notify(payload.message || "Não foi possível contratar este jogador."); return; }
    notify(`${selectedPlayer.name} contratado com segurança pela API.`);
  ...
  async function settleOrder(order: AdminOrder) {
    ...
    if (!response.ok) { notify("Não foi possível atualizar o pedido."); return; }
  ...
  async function requestCatalogSync() {
    ...
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) { notify(payload.message || "Não foi possível iniciar a sincronização."); return; }
    notify("Sincronização enfileirada. Acompanhe o status no painel.");
  }
  ...
  const renderOverview = () => (
    <>
      <section className="welcome-row">
        <div>
          <p className="eyebrow">QUINTA-FEIRA · 24 JUL 2025</p>
          <h1>{t.greeting} <span className="wave">✦</span></h1>
          <p className="muted">Sua estratégia está em dia. O próximo jogo começa em 18h.</p>
        </div>
      ...
      <section className="metric-grid">
        ...
        <article className="metric-card">
          ...
          <div><span className="metric-label">Força do elenco</span><strong>86.4</strong><span className="metric-foot">Top 12% da liga</span></div>
          <span className="metric-trend">↗ 4.8%</span>
        </article>
        <article className="metric-card">
      ...
      <section className="content-grid">
        <article className="panel match-panel">
          ...
          <div className="match-card">
            ...
            <div className="match-team away"><span className="crest crest-coral">RB</span><strong>Rio Bulls</strong><small>Fora</small></div>
          </div>
          <div className="match-footer"><span>Formação sugerida <strong>4-3-3 ofensivo</strong></span><button onClick={() => notify("Escalação salva para a próxima rodada.")} type="button">Ver escalação <span>→</span></button></div>
        </article>

        <article className="panel performance-panel">
          <div className="panel-heading"><div><span className="eyebrow">DESEMPENHO</span><h2>Últimos 5 jogos</h2></div><button className="ghost-button" type="button">Ver tudo →</button></div>
          <div className="performance-chart"><div className="chart-grid"><span /><span /><span /><span /></div><svg viewBox="0 0 480 150" preserveAspectRatio="none" aria-label="Gráfico de desempenho" role="img"><defs><linearGradient id="chartFill" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor="#72ddca" stopOpacity=".38" /><stop offset="100%" stopColor="#72ddca" stopOpacity="0" /></linearGradient></defs><path d="M0,118 C38,108 55,125 87,96 S133,74 159,87 S195,112 218,67 S254,45 286,63 S326,79 348,39 S385,15 415,45 S448,40 480,10 L480,150 L0,150 Z" fill="url(#chartFill)" /><path d="M0,118 C38,108 55,125 87,96 S133,74 159,87 S195,112 218,67 S254,45 286,63 S326,79 348,39 S385,15 415,45 S448,40 480,10" fill="none" stroke="#72ddca" strokeWidth="3" /></svg><div className="chart-labels"><span>18 JUL</span><span>19 JUL</span><span>20 JUL</span><span>21 JUL</span><span>22 JUL</span><span>HOJE</span></div></div>
      ...
      <section className="content-grid bottom-grid">
        ...
        <article className="panel activity-panel"><div className="panel-heading"><div><span className="eyebrow">ATIVIDADE</span><h2>Últimas movimentações</h2></div><button className="ghost-button" onClick={() => selectSection("transfers")} type="button">Mercado →</button></div><div className="activity-list"><div className="activity-item"><span className="activity-avatar cyan">MA</span><div><strong>Você contratou M. Azevedo</strong><small>Hoje, 09:42 · Mercado de transferências</small></div><b>- 2.400 FC</b></div><div className="activity-item"><span className="activity-avatar gold">◈</span><div><strong>Recompensa de partida recebida</strong><small>Ontem, 21:08 · Vitória contra United FC</small></div><b className="green">+ 1.200 FC</b></div><div className="activity-item"><span className="activity-avatar purple">✦</span><div><strong>Upgrade de centro de treino</strong><small>Ontem, 18:25 · Nível 06 desbloqueado</small></div><b>- 4.800 FC</b></div></div></article>
      </section>
      ...
      <section className="content-grid social-grid">
        <article className="panel market-news-panel">
          ...
          <div className="market-news-list">{portalData.news.map((news) => <div className="market-news-item" key={news.id}><span className={`news-kind ${news.kind}`}>{news.kind === "oferta" ? "↗" : news.kind === "partida" ? "◉" : "✦"}</span><div><strong>{news.title}</strong><small>{news.detail}</small><span>{countryFlag(news.country)} {news.team} · {news.time}</span></div><button onClick={() => notify(news.kind === "oferta" ? "Negociação aberta para sua decisão." : "Notícia marcada como lida.")} type="button">{news.kind === "oferta" ? "Ver oferta" : "Abrir"}</button></div>)}</div>
        </article>
  ...
  const renderSquad = () => (
    <section className="page-section"><div className="page-title-row"><div><span className="eyebrow">CENTRO DO MANAGER</span><h1>Meu elenco</h1><p className="muted">Evolua seu time, equipe itens e encontre a combinação vencedora.</p></div><button className="primary-button" onClick={() => selectSection("transfers")} type="button">+ Buscar jogador</button></div><div className="squad-summary"><div><span>Força geral</span><strong>86.4</strong><small>+4.8% esta temporada</small></div><div><span>Valor do elenco</span><strong>94.850 <em>FC</em></strong><small>6 jogadores especiais</small></div><div><span>Formação atual</span><strong>4-3-3</strong><small>Ofensivo · pressão alta</small></div></div><div className="section-toolbar"><div className="filter-pills">{["Todos", "Extremos", "Diamante", "Ouro"].map((item) => <button className={filter === item ? "active" : ""} key={item} onClick={() => setFilter(item)} type="button">{item}</button>)}</div><button className="ghost-button" type="button">Ordenar: força ↓</button></div><div className="full-card-grid">{filteredPlayers.map((player) => <PlayerCard key={player.id} player={player} onSelect={() => setSelectedPlayer(player)} />)}</div></section>
  );

  const renderTransfers = () => (
    <section className="page-section"><div className="page-title-row"><div><span className="eyebrow">MERCADO GLOBAL</span><h1>Transferências</h1><p className="muted">Os melhores jogadores da rodada, atualizados a cada 15 minutos.</p></div><div className="market-status"><i /> Mercado aberto <span>·</span> 248 ofertas</div></div><div className="market-feature"><div><span className="eyebrow">DESTAQUE DA SEMANA</span><h2>Seu próximo craque pode estar aqui.</h2><p>Jogadores Extremos têm habilidades únicas, evolução acelerada e impacto direto nas grandes decisões.</p><button className="light-button" onClick={() => setFilter("Extremos")} type="button">Explorar Extremos <span>→</span></button></div><div className="feature-orbit"><span>92</span><small>ATA</small><b>L. ANDRADE</b><em>EXTREMO</em></div></div><div className="section-toolbar"><div className="filter-pills"><button className="active" type="button">Todos</button><button type="button">Atacantes</button><button type="button">Meias</button><button type="button">Defensores</button></div><div className="search-box">⌕ <input aria-label="Buscar jogador" placeholder="Buscar jogador ou clube" /></div></div><div className="full-card-grid">{dashboardPlayers.map((player) => <PlayerCard key={player.id} player={player} onSelect={() => setSelectedPlayer(player)} />)}</div></section>
  );
  ...
  const renderStore = () => (
    <section className="page-section">
      ...
      <div className="secure-note"><span>▣</span><div><strong>Pedido PIX protegido</strong><small>{pendingPixOrder ? `Pedido #${pendingPixOrder.id} pendente. As moedas só entram após webhook ou conferência admin.` : "O pedido será confirmado somente após a aprovação do pagamento."}</small></div><span className="payment-brand">PIX</span></div>
    </section>
  );

  const renderAdmin = () => (
    <section className="page-section admin-section"><div className="page-title-row"><div><span className="eyebrow">CONTROL ROOM</span><h1>Administração</h1><p className="muted">Acompanhe receita, operações e saúde da plataforma em um só lugar.</p></div><div className="admin-actions"><button className="secondary-button" onClick={requestCatalogSync} type="button">Sincronizar catálogo <span>↻</span></button><span className="admin-live"><i /> Dados protegidos pela API</span></div></div><div className="admin-metrics"><div><span>Pedidos pendentes</span><strong>{adminSnapshot?.pending_orders ?? "—"}</strong><small>Liquidação manual auditada</small></div><div><span>Usuários ativos</span><strong>{adminSnapshot?.active_users?.toLocaleString("pt-BR") ?? "—"}</strong><small className="positive">Conta e sessão no servidor</small></div><div><span>Jogadores catalogados</span><strong>{adminSnapshot?.players?.toLocaleString("pt-BR") ?? "—"}</strong><small>Fonte de futebol configurada</small></div><div><span>Última sincronização</span><strong>{adminSnapshot?.last_sync?.status ?? "—"}</strong><small>{adminSnapshot?.last_sync?.records_processed ? `${adminSnapshot.last_sync.records_processed} registros` : "Aguardando API"}</small></div></div><div className="admin-layout"><article className="panel admin-table-panel"><div className="admin-tabs">{["Pedidos", "Jogadores", "Usuários", "Auditoria"].map((tab) => <button className={adminTab === tab ? "active" : ""} key={tab} onClick={() => setAdminTab(tab)} type="button">{tab}</button>)}</div>{adminTab === "Pedidos" ? <div className="table-wrap"><table><thead><tr><th>Pedido</th><th>Usuário</th><th>Produto</th><th>Valor</th><th>Status</th><th /></tr></thead><tbody>{adminOrders.length ? adminOrders.map((order) => <tr key={order.id}><td><strong>#{order.id}</strong><small>PIX pendente</small></td><td>{order.user?.name ?? order.user?.email ?? "Manager"}</td><td>{order.kind === "coins" ? `${(order.payload?.coins ?? 0).toLocaleString("pt-BR")} FC coins` : "Jogador"}</td><td>R$ {(order.total_cents / 100).toFixed(2).replace(".", ",")}</td><td><span className="status pending">Pendente</span></td><td><button className="table-action" onClick={() => settleOrder(order)} type="button">Marcar paga</button></td></tr>) : <tr><td colSpan={6}>Nenhum pedido PIX pendente ou API indisponível.</td></tr>}</tbody></table></div> : <div className="empty-admin"><span>✦</span><strong>{adminTab} conectado à API</strong><p>Esta visão busca dados protegidos do Laravel e registra ações administrativas em auditoria.</p></div>}</article><aside className="panel health-panel"><div className="panel-heading"><div><span className="eyebrow">SAÚDE DA PLATAFORMA</span><h2>Serviços</h2></div><span className="health-dot" /></div><div className="health-list"><div><span>API principal</span><strong>{adminSnapshot ? "Online" : "—"}</strong><i className="ok" /></div><div><span>Supabase</span><strong>Server-side</strong><i className="ok" /></div><div><span>Fila de pagamentos</span><strong>{adminSnapshot?.pending_orders ?? "—"}</strong><i className="ok" /></div><div><span>Sync de dados</span><strong>{adminSnapshot?.last_sync?.status ?? "—"}</strong><i className="ok" /></div></div><button className="health-button" onClick={requestCatalogSync} type="button">Executar sincronização <span>→</span></button></aside></div></section>
  );
  ...
  return (
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <div className="club-card"><span className="club-crest">FC</span><div><strong>FC Aurora</strong><small>Manager · Nível 24</small></div><button aria-label="Abrir menu do clube" type="button">⌄</button></div>
        ...
        <button className="secondary-nav" onClick={() => notify("Em breve: centro de treinamento.")} type="button"><span>◈</span> Centro de treino <em>3</em></button>
        <button className="secondary-nav" onClick={() => notify("Em breve: conquistas e recompensas.")} type="button"><span>♧</span> Conquistas</button>
        <div className="sidebar-bottom"><div className="sidebar-help"><span>?</span><div><strong>Precisa de ajuda?</strong><small>Fale com nosso suporte</small></div></div><div className="sidebar-user"><span className="user-avatar">{managerInitials}</span><div><strong>{managerName}</strong><small>Conta verificada</small></div><button className="logout-button" aria-label="Sair da conta" onClick={logout} type="button">Sair</button></div></div>
      </aside>
      <div className="main-area">
        <header className="topbar"><div className="mobile-brand"><span className="brand-mark">FC</span> FC MANAGER</div><div className="topbar-spacer" /><button className="icon-button" aria-label="Notificações" onClick={() => notify("Você não tem novas notificações.")} type="button">♢<i /></button><button className="icon-button" aria-label="Ajuda" onClick={() => notify("Central de ajuda aberta.")} type="button">?</button><button className="language-toggle" onClick={() => setLocale(locale === "pt" ? "en" : "pt")} type="button">{locale === "pt" ? "PT" : "EN"} <span>⌄</span></button><button className="top-user" onClick={logout} title="Sair da conta" type="button"><span className="user-avatar">{managerInitials}</span><span><strong>{managerName}</strong><small>Manager · Sair</small></span><span>↪</span></button></header>
